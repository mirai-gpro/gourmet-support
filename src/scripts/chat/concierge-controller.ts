...
// コンシェルジュモード専用コントローラー（チャットモードから完全に独立）
...
/**
 * ConciergeController - 完全独立型のコンシェルジュモードコントローラー
 * ChatControllerとは完全に分離して開発・テストが可能
 */
export class ConciergeController {
  ...
  // ★コンシェルジュモード固定
  private readonly MODE = 'concierge';
  ...
  constructor(container: HTMLElement, apiBase: string) {
    ...
    console.log(`[ConciergeController] Initialized in CONCIERGE mode`);
    this.audioManager = new AudioManager(this.isIOS);
    this.ttsPlayer = new Audio();

    // DOM要素取得（コンシェルジュモード用のIDに対応）
    ...
    this.els = {
      ...
      micBtn: query('#micBtnFloat') || query('#micBtn'),
      speakerBtn: query('#speakerBtnFloat') || query('#speakerBtn'),
      ...
      reservationBtn: query('#reservationBtnFloat') || query('#reservationBtn'),
  ...
  private async init() {
    console.log('[ConciergeController] Starting initialization...');
    ...
    console.log('[ConciergeController] Initialization completed');
  }

  private async resetAppContent() {
    console.log('[ConciergeController Reset] Starting soft reset...');
    ...
    if (this.els.reservationBtn) this.els.reservationBtn.classList.remove('visible');
    ...
    console.log('[ConciergeController Reset] Soft reset completed');
  }
  ...
  private bindEvents() {
    ...
    if (floatingButtons) {
      this.els.userInput.addEventListener('focus', () => {
        setTimeout(() => floatingButtons.classList.add('keyboard-active'), 300);
      });
    ...
    const resetHandler = async () => {
      console.log('[ConciergeController] ===== RESET EVENT RECEIVED =====');
    ...
    document.addEventListener('gourmet-app:reset', resetWrapper, { once: true });
    console.log('[ConciergeController] Reset event listeners registered');
  }
  ...
  private initSocket() {
    ...
    this.socket.on('connect', () => {
      console.log('[ConciergeController] Socket connected');
    });
    ...
    try {
      ...
      const res = await fetch(`${this.apiBase}/api/session/start`, {
        ...
        body: JSON.stringify({
          ...
          mode: this.MODE  // ★ concierge モード固定
        })
      });
      const data = await res.json();
      this.sessionId = data.session_id;

      console.log('[ConciergeController] New concierge session created:', this.sessionId);
      ...
      console.error('[ConciergeController Session] Initialization error:', e);
    }
  }

  // 残りのメソッド（toggleRecording, sendMessage, speakTextGCP等）は
  // ChatControllerと同じロジックを使用しますが、完全に独立したコード
  ...
  private async toggleRecording() {
    ...
    if (this.socket && this.socket.connected) {
      ...
      if (this.els.avatarStatus) {
        this.updateAvatarStatus('listening', '聞いています');
      }
  ...
  private stopStreamingSTT() {
    ...
    if (this.els.avatarStatus) {
      this.updateAvatarStatus('ready', '待機中');
    }
  }
  ...
  private async sendMessage() {
    ...
    if (this.els.avatarStatus) {
      this.updateAvatarStatus('thinking', '考え中');
    }

    try {
      // ★コンシェルジュモードとしてメッセージ送信
      ...
      if (data.shops && data.shops.length > 0) {
        ...
        (async () => {
          try {
            ...
            if (this.els.avatarStatus) {
              this.updateAvatarStatus('speaking', '話しています');
            }
  ...
  private hideWaitOverlay() {
    ...
    if (this.els.waitOverlay) {
      ...
      setTimeout(() => {
        if (this.els.waitVideo) this.els.waitVideo.pause();
      }, 500);
    }
  }
  ...
  private openReservationModal() {
    ...
    document.dispatchEvent(new CustomEvent('openReservationModal', {
      detail: { shops: this.currentShops }
    }));
  }
  ...
  private stopAllActivities() {
    ...
    if (window.innerWidth < 1024) {
      setTimeout(() => {
        this.els.chatArea.scrollIntoView({ behavior: 'smooth', block: 'start' });
      }, 100);
    }
  }
  ...
  private updateAvatarStatus(status: string, text: string) {
    if (this.els.avatarStatus) {
      const indicator = this.els.avatarStatus.querySelector('.status-indicator') as HTMLElement;
      const statusText = this.els.avatarStatus.querySelector('.status-text') as HTMLElement;
